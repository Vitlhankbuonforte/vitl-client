    ...
    <div v-if="value">
        <div class="flex gap-3">
            <div>
                ...
                <div v-if="!pulse || (value.currentField.field !== 'SALES' && value.currentField.field !== 'DM_RM_POINTS')"
                    class="mt-2">
                    ...
                    <template v-for="col of columns" :key="col.field">
                        <div v-if="col.field !== value.currentField.field" class="mb-2">
                            {{ col.title }}:
                        </div>
                    </template>
                </div>
            ...
            <div>
                ...
                <div v-if="pulse">{{ viewOption === 'Numbers' ? value['SALES'] :
                    Math.round(value['DM_RM_POINTS']) }}</div>
                <div v-if="!pulse || (value.currentField.field !== 'SALES' && value.currentField.field !== 'DM_RM_POINTS')"
                    class="mt-2">
                    ...
                    <template v-for="(col, index) of columns" :key="col.field">
                        <div v-if="col.field !== value.currentField.field" class="mb-2">
                            {{ viewOption === 'Percentages' && index !== 0 ? (value[col.field] || 0)
                                + '%' :
                                (value[col.field] || 0) }}
                        </div>
                    </template>
